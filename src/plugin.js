...
class VR extends Plugin {
  constructor(player, options) {
    ...
    this.handleVrDisplayActivate_ = videojs.bind(
      this,
      this.handleVrDisplayActivate_
    );
    this.handleVrDisplayDeactivate_ = videojs.bind(
      this,
  ...
  changeProjection_(projection) {
    ...
    if (projection === "AUTO") {
      ...
      if (
        ...
        const autoProjection = utils.getInternalProjectionName(
          this.player_.mediainfo.projection
        );
    ...
    if (window.navigator.getVRDisplays) {
      ...
      window.navigator.getVRDisplays().then((displays) => {
        ...
        if (!this.controls3d) {
          ...
          this.canvasPlayerControls = new CanvasPlayerControls(
            this.player_,
            this.renderedCanvas
          );
        }
